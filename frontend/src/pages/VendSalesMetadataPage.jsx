                ...
                <tbody className="divide-y divide-gray-100">
                  {records.map((record) => (
                    <tr key={record.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-3 py-2 whitespace-nowrap">{formatNullable(record.billToName)}</td>
                      <td className="px-3 py-2">{formatNullable(record.billToAccount)}</td>
                      <td className="px-3 py-2">{formatNullable(record.siteNumber)}</td>
                      <td className="px-3 py-2">{formatNullable(record.businessUnit)}</td>
                      <td className="px-3 py-2">{formatNullable(record.txnSource)}</td>
                      <td className="px-3 py-2">{formatNullable(record.txnType)}</td>
                      <td className="px-3 py-2">{formatNullable(record.rateIsCorporate)}</td>
                      <td className="px-3 py-2">{formatNullable(record.recActivityNameBank)}</td>
                      <td className="px-3 py-2">{formatNullable(record.subinventory)}</td>
                      <td className="px-3 py-2">{formatNullable(record.integrationSource)}</td>
                      <td className="px-3 py-2">{formatNullable(record.distributionAccId)}</td>
                      <td className="px-3 py-2">{formatNullable(record.recActivityNameCash)}</td>
                      <td className="px-3 py-2">{formatNullable(record.customerType)}</td>
                      <td className="px-3 py-2">{formatNullable(record.region)}</td>
